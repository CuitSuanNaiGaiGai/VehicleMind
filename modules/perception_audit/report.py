...
def render_markdown(summary: dict) -> str:
    ...
    for domain, label in (("cabin", "舱内"), ("road", "舱外")):
        ...
        if domain == "cabin":
            ...
            lines.append(f"- 状态输出时长（秒，按视频时间戳近似）："
...
def render_html(summary: dict, *, results: list[dict] | None = None,
                video_links: dict[str, str] | None = None) -> str:
    def metric(label: str, numerator: int, denominator: int, unit: str) -> str:
        ...
        return (f'<article class="metric"><span>{escape(label)}</span>'
    ...
    for domain, label in (("cabin", "舱内"), ("road", "舱外")):
        ...
        cards.append(f'<section class="domain"><div class="section-title"><span class="chip">'
                     f'{escape(label)}</span><h2>{escape(label)}感知</h2></div>'
                     '<section class="metric-grid">'
                     + metric("视频处理成功", videos["successful"],
                              videos["attempted"], "条视频")
                     + metric("有效结构化输出", frames["valid_output"],
                              frames["processed"], "帧")
                     + '</section></section>')
        if domain == "cabin":
            details = (
        ...
        failures = "、".join(f"{name} {count} 条" for name, count in
                            data["failures"].items()) or "无"
        rows.append(f"<tr><th>{escape(label)}</th><td>{escape(details)}</td>"
                    f"<td>{escape(failures)}</td></tr>")
    limitations = "".join(f"<li>{escape(text)}</li>" for text in
                          summary["limitations"])
    per_video = ""
    if results is not None:
        ...
        for row in results:
            if row["domain"] == "cabin":
                detail = (f"驾驶员状态帧数：{row.get('state_counts', {})}；"
                          f"状态时长（秒，近似）：{row.get('state_duration_seconds', {})}")
            else:
                detail = f"目标输出（按帧累计）：{row.get('object_counts', {})}"
            status = "完成" if row["status"] == "success" else _failure_reason(row.get("error"))
            links = []
            ...
            if relative and not Path(relative).is_absolute():
                for point in row.get("samples", [])[:3]:
                    ...
                    links.append(f'<a href="{escape(href, quote=True)}">'
                                 f'{seconds:g} 秒</a>')
            if links:
                detail += "；本地视频时间点："
            video_rows.append(
        ...
        per_video = (
            ...
            + "".join(video_rows) + '</tbody></table></section>'
        )
    return (
        ...
        + per_video
        + '<section class="detail"><h2>解释边界</h2><ul>' + limitations
