...
class RoadObjectDetector:
    ...
    def __init__(
        ...
        if not self.target_class_ids:
            ...
            raise RuntimeError(
                "No requested road-object classes "
                "exist in the loaded detection model."
            )

        resolved_names = [
            self.model.names[class_id]
            for class_id
            in self.target_class_ids
        ]
