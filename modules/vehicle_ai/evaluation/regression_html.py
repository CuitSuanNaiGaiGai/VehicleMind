...
def render_html(comparison: dict[str, Any]) -> str:
    ...
    for provider in ("qwen", "glm"):
        ...
        for label, key, cost_key in (
            ...
            rows.append(
                "<tr>"
                f"<th>{escape(provider.upper())}</th><td>{label}</td>"
    ...
    deltas = "".join(
        ...
        f"<p>{pair['task_success_delta_percentage_points']:+.1f}<small> 个百分点</small></p>"
        "</article>"
    ...
    for provider in ("qwen", "glm"):
        ids = comparison["providers"][provider]["after"]["unresolved_case_ids"]
        if ids:
            links = "、".join(
...
<section class="panel"><h2>在线 Agent 前后对照</h2><div class="scroll"><table><thead><tr><th>模型</th><th>版本</th><th>Task Success（任务成功）</th><th>Mechanical Pass（机械通过）</th><th>Tool / Argument / State（工具/参数/状态匹配）</th><th>错误</th><th>p50 / p95 延迟</th><th>请求数</th><th>输入 / 输出 token</th><th>Agent 费用估算</th><th>Qwen 审核费用估算</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div>
...
<p class="muted">原始运行与逐条审核保存在本机 Git 忽略的 runs/；可根据报告所列 run ID 与 SHA-256 在生成环境核验。本 HTML 仅包含汇总。</p></section>
</main></body></html>"""
