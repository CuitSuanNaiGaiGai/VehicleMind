...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--output-root", type=Path,
                        default=Path("runs/perception_audit"), help="本地运行根目录")
    parser.add_argument("--sample-interval", type=_positive_int, default=30,
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        output = run_audit(
            args.cabin_dir, args.road_dir, args.output_root,
            sample_interval=args.sample_interval,
        )
    except (FileNotFoundError, ModuleNotFoundError, ImportError) as exc:
        print(f"错误：模型或感知依赖不可用（{type(exc).__name__}）。"
              "请先安装 perception extra 并核对本地模型。", file=sys.stderr)
        return 2
    except (ValueError, FileExistsError) as exc:
        print(f"错误：输入或运行配置无效（{type(exc).__name__}）。"
              "请核对视频目录、数量及输出目录。", file=sys.stderr)
        return 2
    except Exception as exc:
        print(f"错误：核验未完成（{type(exc).__name__}）。"
              "请检查未标记 .complete 的本地运行目录。", file=sys.stderr)
        return 2
