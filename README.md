...
| **可复现结果** | 离线回放保存配置快照、语义 trace、断言和中文 HTML 报告；在线模型候选 pilot 单独记录真实 API 轨迹 |
...
详细评测边界与失败案例见上述技术报告。原始在线轨迹保存在本地被 Git 忽略的 `runs/` 中，仓库公开的是场景、运行入口和审查记录，而非那次运行的全部原始请求。当前有候选评测场景，但**尚无人工冻结的 Golden Set、正式 Agent 成功率或舱内外小样本精度**；不会借用其他项目的结果填入本页。
