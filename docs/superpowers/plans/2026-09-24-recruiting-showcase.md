...
- [x] 先为表格覆盖的舱内、舱外、上下文/事件、Agent、工具门禁、回放/测试添加文档契约断言；运行目标测试确认红灯。
- [x] 核查对应源码后改写技术栈表。明确 MediaPipe、YOLOPv2、ONNX Runtime 与 Qwen/GLM 是第三方能力，项目内工作是管线、适配、状态处理、编排、门禁和验证；每行增加可点击代码或技术说明链接。
- [x] 运行目标测试、链接存在性检查和 `git diff --check`；满足验收后更新 `todolist.md`，提交并推送。
...
- [x] 选定现有失败或拒绝场景，先执行回放并核对其断言与报告；若没有合适场景，先写一个失败的 smoke test，再增加最小 YAML 场景，不造假截图。
- [x] 在 `tests/test_readme_showcase.py` 增加证据契约：成功/失败入口都可达、Agent 数值具有分子/分母和 AI 审核边界、未出现未完成的感知精度与性能成绩；执行红灯。
- [x] README 用并排或连续短案例展示期望状态、实际结果及对应证据；公开文案不得含密钥、本机绝对路径或个人轨迹。运行目标测试、场景 smoke、`git diff --check`；按验收更新清单并提交推送。
